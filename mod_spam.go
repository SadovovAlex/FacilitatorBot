...
func (b *Bot) isSpam(text string) bool {
	...
	patternDefinitions := map[string]string{
		...
		`(?i)(крипто|биткоин|эфириум|блокчейн|nft|инвест|траст|forex|форекс|fx|трейд)`: "финансовый спам",
	...
	for pattern, description := range patternDefinitions {
		if matched, _ := regexp.MatchString(pattern, text); matched {
			log.Printf("🚨 СПАМ-детект: '%s' (%s) в тексте: %s", description, pattern, text)
			return true
		}
	}
	...
	if b.containsExcessiveEmoji(lowerText) {
		log.Printf("🚨 СПАМ-детект: избыточное количество эмодзи в тексте: %s", text)
		return true
	}

	if b.hasSuspiciousWordCombinations(lowerText) {
		log.Printf("🚨 СПАМ-детект: подозрительная комбинация слов в тексте: %s", text)
		return true
	}

	// Проверка на подозрительные URL
	if b.containsSuspiciousURL(text) {
		log.Printf("🚨 СПАМ-детект: подозрительный URL в тексте: %s", text)
		return true
	}
